...
CREATE TABLE agencies (
                          ...
                          address_id INT,
                          FOREIGN KEY (address_id) REFERENCES addresses(UniqueID)
);
...
CREATE TABLE houses (
                        ...
                        owner_id INT,
                        FOREIGN KEY (address_id) REFERENCES addresses(UniqueID),
                        FOREIGN KEY (owner_id) REFERENCES owners(UniqueID)
);
...
CREATE TABLE rent_info (
                           ...
                           house_id INT,
                           FOREIGN KEY (owner_id) REFERENCES owners(UniqueID),
                           FOREIGN KEY (customer_id) REFERENCES customers(UniqueID),
                           FOREIGN KEY (agency_id) REFERENCES agencies(UniqueID),
                           FOREIGN KEY (house_id) REFERENCES houses(UniqueID)
);
...
ALTER TABLE agencies drop constraint check_phone_number;
...
select * from addresses join houses h on addresses.UniqueID = h.address_id where address_id between 5 and 10;
...
select *from houses join owners o on o.UniqueID = houses.owner_id join addresses a on a.UniqueID = houses.address_id where furniture=true;
...
SELECT *, (check_out - check_in) AS rental_period from rent_info ORDER BY rental_period DESC LIMIT 1;
-- 22.Эн кымбат уйду жана анын ээсин чыгарыныз.
select * from  houses join owners o on o.UniqueID = houses.owner_id where houses.price = (select max(houses.price)from houses);
